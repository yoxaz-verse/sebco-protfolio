...
export function PageComponent(Title: any) {
  ...
  const {
    ...
    onOpenChange: onOpenChangeContent,
  } = useDisclosure();
      ...
      <div className="w-full flex p-[1rem] justify-between">
        <h2 className="text-2xl font-bold">
          {`${Title.Title.charAt(0).toUpperCase() + Title.Title.slice(1)}${
            Title.Title.toLowerCase() === "gallery" &&
            Title.Title.toLowerCase() === "jobs" &&
            Title.Title.toLowerCase() === "blogs" &&
        ...
        <div className="flex flex-row gap-2">
          <AddModal
            ...
            title={Title.Title}
          />
          <Button onClick={generateRandomId} className="rounded-xl bg-blue-200">
            Refresh
          </Button>
        </div>
      </div>
