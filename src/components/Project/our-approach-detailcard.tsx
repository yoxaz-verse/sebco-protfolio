  ...
  return (
    <div className="flex justify-center items-center py-8">
      <div className="w-10/12 rounded-lg p-8 glass-effect">
        <div className="w-[55%] text-4xl py-3 text-white font-medium">{ourApproachDetailCard.title}</div>
        <div className="text-xl py-3 text-white">{ourApproachDetailCard.details}</div>
        <div className="flex justify-between w-3/5">
          {ourApproachDetailCard.features.map((feature, index) => (
            <Chip key={index} className="bg-[#FFBD12]">{feature}</Chip>
          ))}
        </div>
        <div className="flex flex-col py-8 text-white w-[28%]">
          <div className="text-xl">{ourApproachDetailCard.address.label}</div>
          <div className="py-2">{ourApproachDetailCard.address.content}</div>
        </div>
